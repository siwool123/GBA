...
<div class="container">
	<div class="row">
		...
		<div class="col-sm-10 p-5 mt-3" style="padding-right:0 !important;">
			<div style="border-bottom:1px solid black;padding-bottom:10px;"><b style="font-size:28px;">비밀번호 찾기</b> 
			<span style="float:right !important;">
			<i class="bi bi-house-door-fill" style="font-size:18px; color:#dddddd"></i> <span class="sep">></span>멤버십<span class="sep">></span>비밀번호 찾기</span>
			</div>	
		<% if(dto.getId() != null && dto.getEmail()== userEmail ){	%>
		<div id="idfindresult" class="text-center my-5 py-5"><b><%= dto.getName() %></b> 님, <br />
		아이디 <b><%= dto.getId() %></b> 에 해당하는 <br /> 비밀번호는 <b><%= dto.getPw() %></b> 입니다. </div>
		...
		<% }else{ JSFunction.alertBack("아이디, 회원이름, 이메일 주소가 존재하지 않습니다. 다시 입력해주세요.", out);} %>
		</div>
	</div>
</div>
